...
## Deferred từ review form-fields-translation-fix (2026-07-24)
- **5 field ở Bước 1 vẫn cùng loại bug vừa fix (nhập vào nhưng không tới PDF), có từ trước, ngoài phạm vi báo lỗi lần này:** `middle_name` và `contact_in_japan` — hoàn toàn chết, không có widget PDF nào tương ứng (không nằm trong `VISA_TEXT_MAP`). `nationality`, `occupation`, `passport_expiry_date` — user gõ vào Bước 1 nhưng `_build_info()` lấy dữ liệu từ nguồn khác thay thế (nationality hardcode "vietnam", occupation suy ra từ `employment_type` ở bước hồ sơ trước, passport_expiry_date lấy từ `profile.passport_expiry`) — nên trên PDF vẫn CÓ giá trị (không trống như tester báo), chỉ là giá trị đó không phải cái user vừa gõ, khiến ô nhập ở Bước 1 gây hiểu lầm là "chỉnh sửa được".
- **`Step2` (`tripFormData.accommodation_address`, `contact_phone`...) bị `FormsRequest` (backend) âm thầm bỏ qua hoàn toàn** — không có field `trip_details` nào trong schema nhận nó. PDF thật luôn lấy `accommodation_address` từ Step1, nhưng màn xem lại Bước 5 lại hiện giá trị từ Step2 — nếu user điền 2 nơi khác nhau ở Step1 và Step2, màn preview và PDF thật sẽ lệch nhau mà không có cảnh báo nào.
