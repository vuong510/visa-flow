...
- Checklist is deterministic (hardcoded per employment_type in `api/ai.py`), NOT a Haiku call
- File uploads: multipart POST, saved to `uploads/{app_id}/`, image files sent to Sonnet for review
...
POST  /api/application/{id}/documents/{doc_id}/review  → Sonnet (images only)
